...
export default function Footer() {
    return (
        <motion.footer
            ...
            className="w-full border-t border-[#333842]/20 bg-[#08090b]"
        >
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-12 px-8 md:px-12 py-16 max-w-screen-2xl mx-auto">
                {/* Brand */}
                <div className="col-span-2 md:col-span-1">
                    <div className="text-xl font-extrabold mb-4 tracking-tighter uppercase font-[family-name:var(--font-outfit)]">
                    ...
                    <div className="flex gap-2 max-w-xs">
                        <input
                            ...
                            className="flex-1 bg-[#0d0e10] border border-[#333842]/30 rounded-lg px-3 py-2 text-xs text-[#e8e6e3] placeholder:text-[#666] focus:outline-none focus:border-white/20 transition-colors"
                        />
                        <button className="accent-button px-3 py-2 rounded-lg text-xs font-bold flex-shrink-0">
                            Subscribe
                        </button>
                    </div>
                ...
                {footerColumns.map((col) => (
                    <div key={col.title} className="flex flex-col gap-3 font-mono text-[10px] uppercase tracking-widest">
                        <div className="text-[#e8e6e3] mb-1 text-[11px]">{col.title}</div>
                        {col.links.map((link) =>
                            link.href.startsWith("/") ? (
                                <Link
                                    key={link.label}
                                    href={link.href}
                                    className="text-[#666] hover:text-white transition-colors underline-offset-4 hover:underline"
                                >
                                    {link.label}
                                </Link>
                            ) : (
            ...
            {/* Bottom Bar */}
            <div className="px-8 md:px-12 py-6 border-t border-[#333842]/20 flex flex-col md:flex-row justify-between items-center gap-4 max-w-screen-2xl mx-auto">
                ...
                <div className="flex gap-5">
                    <a href="#" className="text-[#444] hover:text-white transition-colors" aria-label="GitHub">
                    ...
                    <a href="#" className="text-[#444] hover:text-white transition-colors" aria-label="Twitter / X">
                    ...
                    <a href="#" className="text-[#444] hover:text-white transition-colors" aria-label="Discord">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M20.317 4.37a19.791 19.791 0 00-4.885-1.515.074.074 0 00-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 00-5.487 0 12.64 12.64 0 00-.617-1.25.077.077 0 00-.079-.037A19.736 19.736 0 003.677 4.37a.07.07 0 00-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 00.031.057 19.9 19.9 0 005.993 3.03.078.078 0 00.084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 00-.041-.106 13.107 13.107 0 01-1.872-.892.077.077 0 01-.008-.128 10.2 10.2 0 00.372-.292.074.074 0 01.077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 01.078.01c.12.098.246.198.373.292a.077.077 0 01-.006.127 12.299 12.299 0 01-1.873.892.077.077 0 00-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 00.084.028 19.839 19.839 0 006.002-3.03.077.077 0 00.032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 00-.031-.03z"/></svg>
                    </a>
                </div>
